...
import err404 from "../public/404err.png";
...
const MySpacePage = ({
  ...
  const handleTagSearch = (tag: string, e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/SearchPage?searchValue=${encodeURIComponent(tag)}`);
  };
      ...
      <div className="z-0 grid grid-cols-1 md:grid-cols-3 gap-6 max-[431px]:gap-4 p-4 max-[431px]:p-2">
        {currentPosts?.length > 0 ? (
            ...
            return (
              ...
              >
                ...
                <div className="text-right mt-4">
                  <form
                    onSubmit={(e) => handleTagSearch(post.tag, e)}
                    className=""
                  >
                    <button
                      type="submit"
          ...
          <div className="flex w-screen justify-center">
            <Image src={err404} alt="loading" width={300} height={300} />
          </div>
        )}
      </div>
    </div>
